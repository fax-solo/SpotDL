  ...
  const doFetch = useCallback(async (albumId: string) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/spotify'), {
        ...
        body: JSON.stringify({ url: `https://open.spotify.com/album/${albumId}` }),
      })
      if (!res.ok) {
        const errBody = await res.json().catch(() => null)
        throw new Error(errBody?.error || `Failed to load album (${res.status})`)
      }
      const data = await res.json()
      if (!data.tracks || data.tracks.length === 0) {
        throw new Error('This album has no tracks or could not be read')
      }
      setCollection(data)
    } catch (e) {
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-accent uppercase tracking-widest">Album</span>
          </div>
          ...
          <div className="flex items-start gap-2 px-4 py-3 bg-amber-500/10 border border-amber-500/30 rounded-xl">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
              {collection.total_count
                ? `This album has ${collection.total_count} tracks, but only the first ${collection.tracks.length} are shown.`
                : `This album has more than ${collection.tracks.length} tracks — only the first ${collection.tracks.length} are shown.`}{' '}
              Add Spotify client credentials for full album access.
            </p>
          </div>
        )}
        <button
